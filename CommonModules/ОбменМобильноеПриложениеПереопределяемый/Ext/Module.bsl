...
Функция ПолучитьОбъектXDTO(Данные, КоличествоОбъектов)
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("ДокументОбъект.ЗаказПокупателя")
		...
		Для каждого СтрокаТЧ Из Данные.Запасы Цикл
			Если НЕ СтрокаТЧ.ТипНоменклатурыЗапас Тогда
...
Функция ПроверитьВозможностьИзмененияТабличнойЧастиСНоменклатурой(Объект, ТабЧасть)
	...
	Для каждого ТекСтрока Из Объект[ТабЧасть] Цикл
		Если КолонкиТабЧасти.Найти("ТипНоменклатурыЗапас") <> Неопределено
			И НЕ ТекСтрока.ТипНоменклатурыЗапас Тогда
			ПрерватьЗаполнение = Истина;
			Прервать;
		КонецЕсли;
		Если КолонкиТабЧасти.Найти("НомерВариантаКП") <> Неопределено
			И Объект.КоличествоВариантовКП >= 2
			И ТекСтрока.НомерВариантаКП <> Объект.ОсновнойВариантКП Тогда
			ПрерватьЗаполнение = Истина;
			Прервать;
		Конецесли;
