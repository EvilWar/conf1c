...
Процедура ДобавитьВыбранныеПечатныеФормыВоВложения(ПараметрыОтправки, СведенияОШаблоне, Вложения, ПараметрыШаблона)
	...
	Для каждого ВложениеПечатнаяФорма Из СведенияОШаблоне.Вложения Цикл
		...
		Если ВложениеПечатнаяФорма.Статус = "ПечатнаяФорма" И ИмяПараметраВыбраннойПечатнойФормы <> Неопределено Тогда
			...
			// УНФ
			Если МассивОбъектов.Количество() = 0 ИЛИ МассивОбъектов[0] = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			// Конец УНФ
	...
	Если ПараметрыШаблона.ТипШаблона = "Письмо" Тогда
		...
		Для каждого Получатель Из Получатели Цикл
			Если ЗначениеЗаполнено(Получатель.Адрес) Тогда
				ЗначениеПолучателя = Новый Структура("Адрес, Представление, ИсточникКонтактнойИнформации", Получатель.Адрес, Получатель.Представление, Получатель.Контакт);
		...
		Если ВидыКонтактнойИнформацияОбъекта.Количество() > 0 Тогда
			...
			Если КонтактнаяИнформацияОбъектов.Количество() > 0 Тогда
				Для каждого КонтактнаяИнформацияОбъекта Из КонтактнаяИнформацияОбъектов Цикл
					...
					//НоваяСтрока.Представление = КонтактнаяИнформацияОбъекта.Представление + " <" + Строка(КонтактнаяИнформацияОбъекта.Объект) + ">";
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
