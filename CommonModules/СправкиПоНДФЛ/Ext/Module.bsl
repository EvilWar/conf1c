...
Функция ДанныеОДоходах(ТаблицаСотрудников, НалоговыйПериод, Организация, ГоловнаяОрганизация, ОКАТО_КПП, Дата) Экспорт
	...
	|			И (НДФЛСоциальныеВычеты.Период < &ДатаПодачиСведений)
	...
	|				И (ПериодРегистрации МЕЖДУ &НачалоНП И &КонецНП)) КАК РасчетыНалоговыхАгентовСБюджетомПоНДФЛОбороты
	|ГДЕ
	|	ГОД(РасчетыНалоговыхАгентовСБюджетомПоНДФЛОбороты.ПериодРегистрации) < 2012
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ДанныеПодписывающихЛиц(Организация, ДатаАктуальности, ЭтоЮрЛицо = Неопределено, РасшифровкаПодписи = Ложь) Экспорт
	...
	Если ЭтоЮрЛицо Тогда
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СпрОрганизация.ПодписьГлавногоБухгалтера.ФизическоеЛицо КАК ФизЛицо,
		|	СпрОрганизация.ПодписьГлавногоБухгалтера.Должность КАК Должность,
		|	""ГлБух"" КАК Роль
		|ПОМЕСТИТЬ ВТОтветственныеЛица
		|ИЗ
		|	Справочник.Организации КАК СпрОрганизация
		|ГДЕ
		|	СпрОрганизация.Ссылка = &Организация
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	СпрОрганизация.ПодписьРуководителя.ФизическоеЛицо,
		|	СпрОрганизация.ПодписьРуководителя.Должность,
		|	""Рук""
		|ИЗ
		|	Справочник.Организации КАК СпрОрганизация
		|ГДЕ
		...
		Пока Выборка.Следующий() Цикл
			...
			ИначеЕсли Выборка.Роль = "Рук" Тогда
