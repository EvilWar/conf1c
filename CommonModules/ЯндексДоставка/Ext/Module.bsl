...
Функция ЗаданиеПередатьЗаказ(Знач Параметры, Знач АдресХранилища) Экспорт
...
//
Функция ЗаданиеПоискаВариантов(Знач Параметры, Знач АдресХранилища) Экспорт
...
//   * АдресОтправленияЗначенияПолей - Строка,
...
Функция ВариантыДоставкиЗаказа(Знач ПараметрыПоискаСпособаДоставки) Экспорт
	...
	СведенияОбАдресеОтправления = УправлениеКонтактнойИнформацией.СведенияОбАдресе(ПараметрыПоискаСпособаДоставки.АдресОтправленияЗначенияПолей);
	...
	СведенияОбАдресеДоставки = УправлениеКонтактнойИнформацией.СведенияОбАдресе(ПараметрыПоискаСпособаДоставки.АдресДоставкиЗначенияПолей);
...
// Задание осуществляет пакетный вызов методов для актуализации параметров доставки
//
// Параметры:
//  Параметры		 - Структура - ключи:
//   * ПараметрыПоискаСпособаДоставки - Структура - ключи см. в описании ВариантыДоставкиЗаказа(),
//  АдресХранилища	 - Строка - адрес во временном хранилище, куда будут помещены результаты выполнения задания.
//
Функция ЗаданиеАктуализироватьПараметрыДоставки(Знач Параметры, Знач АдресХранилища) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ТаблицаВариантов", ВариантыДоставкиЗаказа(Параметры));
	Результат.Вставить("СкладыСлужбДоставки", СкладыСлужбДоставки(Параметры));
	Результат.Вставить("ИнформацияОСкладе", ИнформацияОСкладе(Параметры));
	ПоместитьВоВременноеХранилище(Результат, АдресХранилища);
...
Функция ЗаданиеПередатьЗаказы(Знач ТаблицаЗаказов, Знач АдресХранилища) Экспорт
...
Функция ИзмененЛокально(Знач ЗаказПокупателяОбъект) Экспорт
	...
	|	ДокументЗаказПокупателя.АдресДоставкиЗначенияПолей
	|ИЗ
	|	РегистрСведений.ЯндексДоставка КАК ЯндексДоставка
...
Процедура ЗаполнитьАдресВрученияЗаказа(Знач ПоляЗапроса, Знач ДанныеЗаказа)
	...
	СведенияОбАдресе = УправлениеКонтактнойИнформацией.СведенияОбАдресе(ДанныеЗаказа.АдресДоставкиЗначенияПолей);
...
Процедура ОбновитьСостоянияИДанныеЗаказов(Знач ДанныеЗаказовИзЯндексДоставки, Знач СвойстваЗаказов, Знач СтатусыСостояния, Знач ОбновленныеЗаказы, Знач НастройкаПодключения)
	...
	Для Каждого ТекДанныеЗаказа Из ДанныеЗаказовИзЯндексДоставки Цикл
		...
		КонецЕсли;
		АдресИзЯндексДоставки = АдресИзЯндексДоставки(ТекДанныеЗаказа.Значение["data"]["recipient"]["address"], НастройкаПодключения, КлассификаторСубъектовРФ);
		ЗаказОбъект.АдресДоставки = АдресИзЯндексДоставки.Представление;
		ЗаказОбъект.АдресДоставкиЗначенияПолей = АдресИзЯндексДоставки.ЗначенияПолей;
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление состояния заказов по данным Яндекс.Доставки'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция РазобратьАдресПоКлассификатору(Знач ДанныеАдреса, Знач КлассификаторСубъектовРФ);
	...
	Результат = УправлениеКонтактнойИнформациейКлиентСервер.СтруктураКонтактнойИнформацииПоТипу(Перечисления.ТипыКонтактнойИнформации.Адрес, "ФИАС");
...
Функция АдресИзЯндексДоставки(Знач ДанныеАдреса, Знач НастройкаПодключения, Знач КлассификаторСубъектовРФ);
	...
	СтруктураПолейАдреса = РазобратьАдресПоКлассификатору(ДанныеАдреса, КлассификаторСубъектовРФ);
	
	СтруктураПолейАдреса.Индекс = ДанныеАдреса["index"];
	СтруктураПолейАдреса.Улица = ДанныеАдреса["street"];
	СтруктураПолейАдреса.Дом = ДанныеАдреса["house"];
	СтруктураПолейАдреса.Корпус = ДанныеАдреса["housing"];
	СтруктураПолейАдреса.Квартира = ДанныеАдреса["apartment"];
	
	Результат.ЗначенияПолей = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(
	СтруктураПолейАдреса,
	СтруктураПолейАдреса.Представление,
	Перечисления.ТипыКонтактнойИнформации.Адрес);
	
	Результат.Представление = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(
...
Функция ПунктыВыдачиЗаказа(Знач ПунктыВыдачиЗаказа, Знач СлужбаДоставки)
	...
	Пока Выборка.Следующий() Цикл
		...
		ПунктВыдачиЗаказаОбъект.Записать();
...
Функция СкладСлужбыДоставки(Знач Выборка, Знач СлужбаДоставки)
	...
	НовыйСкладСлужбыДоставки.Записать();
