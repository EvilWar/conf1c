...
Функция СопоставленныеКонтактыИзПочтовогоСообщения(Знач ПочтовоеСообщение)
	...
	Если ЗначениеЗаполнено(ПочтовоеСообщение.Отправитель.Адрес)
		И АдресаЭП.Найти(НРег(ПочтовоеСообщение.Отправитель.Адрес)) = Неопределено Тогда
		АдресаЭП.Добавить(НРег(ПочтовоеСообщение.Отправитель.Адрес));
	КонецЕсли;
	...
	Для Каждого ТекПолучатель Из ПочтовоеСообщение.Получатели Цикл
		Если Не ЗначениеЗаполнено(ТекПолучатель.Адрес) Тогда
			Продолжить;
		КонецЕсли;
		Если АдресаЭП.Найти(НРег(ТекПолучатель.Адрес)) = Неопределено Тогда
			АдресаЭП.Добавить(НРег(ТекПолучатель.Адрес));
		КонецЕсли;
	КонецЦикла;
...
Процедура ОтключитьЗаданиеЗагрузкиПочты()
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка электронной почты по расписанию'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
