...
Функция ПолучитьПоУмолчаниюСпецификацию(Номенклатура, Характеристика = Неопределено) Экспорт
	...
	СпецификацияПоУмолчанию = Номенклатура.Спецификация;
	
	Если ЗначениеЗаполнено(СпецификацияПоУмолчанию) Тогда
		Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристики") Тогда
			ХарактеристикаПродукции = ?(ЗначениеЗаполнено(Характеристика), Характеристика, Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка());
			Если СпецификацияПоУмолчанию.ХарактеристикаПродукции = ХарактеристикаПродукции Тогда
...
КонецФункции // ПолучитьПоУмолчаниюСпецификацию()
...
Функция ПолучитьСуммуСтрокиТабличнойЧасти(СтруктураДанных) Экспорт
	...
	Если СтруктураДанных.Свойство("ПроцентСкидкиНаценки") ИЛИ СтруктураДанных.Свойство("ДисконтнаяКарта") Тогда
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Если ВыводитьВЗаголовокРасшифровку Тогда
			Если (НЕ УчетПоНесколькимСкладам И ВыборкаЗаписей.ТипСтруктурнойЕдиницы = Перечисления.ТипыСтруктурныхЕдиниц.Склад)
				...
				ПредставлениеСтруктурнойЕдиницы = "";
			Иначе
				Если РаботаСНоменклатуройКлиентСервер.ЕстьРеквизитОбъекта("ЯчейкаПредставление",ВыборкаЗаписей) Тогда
					ПредставлениеСтруктурнойЕдиницы = ПредставлениеСтруктурнойЕдиницы(ВыборкаЗаписей.СтруктурнаяЕдиницаПредставление, ВыборкаЗаписей.ЯчейкаПредставление);
				Иначе
...
//              разузлования.
//
Функция СформироватьСтруктуруСостава() Экспорт
	...
	Структура.Вставить("ТаблицаОперации");
...
// Функция возвращает таблицу операций.
//
// Параметры:
//  СтруктураСостава - Структура состава
//
// Возвращаемое значение:
//  Таблицу значений с операциями.
//
Функция ПолучитьОперацииСпецификации(СтруктураСостава)
	
	Запрос = Новый Запрос; 
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СпецификацииОперации.Операция КАК Операция,
	|	СпецификацииОперации.НормаВремени / СпецификацииОперации.КоличествоПродукции КАК НормаВремени,
	|	СпецификацииОперации.НормаВремени / СпецификацииОперации.КоличествоПродукции * &Количество КАК Длительность,
	|	ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0) / ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения.Коэффициент, 1) КАК УчетнаяЦена,
	|	ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0) * (1 / СпецификацииОперации.КоличествоПродукции) * &Количество / ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения.Коэффициент, 1) КАК Стоимость
	|ИЗ
	...
	Запрос.УстановитьПараметр("ВидЦен",        СтруктураСостава.ВидЦен);
	
	Возврат Запрос.Выполнить().Выгрузить();
	
КонецФункции // ПолучитьОперацииСпецификации()

// Функция возвращает таблицу операций с нормами.
//
// Параметры:
//  СтруктураСостава - Структура состава
//  МенеджерВТ - МенеджерВременныхТаблиц - временные таблицы по
//				   документу
//
// Возвращаемое значение:
...
Функция ПолучитьСоставСпецификации(СтруктураСостава)
	...
	|	ВЫБОР
	|		КОГДА СпецификацииСостав.ЕдиницаИзмерения = ЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения
	|			ТОГДА ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0)
	|		ИНАЧЕ ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0) / ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения.Коэффициент, 1) * ЕСТЬNULL(СпецификацииСостав.ЕдиницаИзмерения.Коэффициент, 1)
	...
	|			И СпецификацииСостав.Характеристика = ЦеныНоменклатурыСрезПоследних.Характеристика
	|ГДЕ
...
Процедура ДобавитьУзел(СтруктураСостава, СтекНоменклатуры, СтекНоменклатурыСтекВходов, НоваяСтрокаСтека, ТекСтрока)
	...
	СтруктураСостава.ТаблицаОперации = ПолучитьОперацииСпецификации(СтруктураСостава);
...
//	ТаблицаСостава - ТаблицаЗначений состава
//	ТаблицаОпераций - ТаблицаЗначений операций.
//  
Процедура ВыполнитьРазузлование(СтруктураСостава, ТаблицаСостава, ТаблицаОпераций) Экспорт
	
	НоваяСтрокаСостава = ТаблицаСостава.Добавить();
	НоваяСтрокаСостава.Номенклатура		= СтруктураСостава.Номенклатура;
	НоваяСтрокаСостава.Характеристика	= СтруктураСостава.Характеристика;
	НоваяСтрокаСостава.ЕдиницаИзмерения	= СтруктураСостава.ЕдиницаИзмерения;
	НоваяСтрокаСостава.Количество		= СтруктураСостава.Количество;
	НоваяСтрокаСостава.Уровень			= СтруктураСостава.Уровень;
	НоваяСтрокаСостава.Узел				= Ложь;
	НоваяСтрокаСостава.УчетнаяЦена		= СтруктураСостава.УчетнаяЦена;
	НоваяСтрокаСостава.Стоимость		= СтруктураСостава.Стоимость;
	...
	Если СтруктураСостава.ТипСтрокиСостава = Перечисления.ТипыСтрокСоставаСпецификации.Узел
	 ...
	 ИЛИ СтруктураСостава.Уровень = 0 Тогда
		...
		НоваяСтрокаСостава.Узел			= Истина;
	 	...
	 	СтрокаОпераций = ТаблицаОпераций.Добавить();
		СтрокаОпераций.Номенклатура		= СтруктураСостава.Номенклатура;
		СтрокаОпераций.Характеристика	= СтруктураСостава.Характеристика;
		СтрокаОпераций.НормаВремени		= СтруктураСостава.Количество;
		СтрокаОпераций.Уровень			= СтруктураСостава.Уровень;
		СтрокаОпераций.Узел				= Истина;
		
	КонецЕсли;
		
	Для Каждого СтрокаТЧ Из СтруктураСостава.ТаблицаОперации Цикл
			
		СтрокаОпераций = ТаблицаОпераций.Добавить();
		СтрокаОпераций.Номенклатура	= СтрокаТЧ.Операция;
		СтрокаОпераций.НормаВремени = СтрокаТЧ.НормаВремени;
		СтрокаОпераций.Уровень		= СтруктураСостава.Уровень + 1;
		СтрокаОпераций.Длительность = СтрокаТЧ.Длительность;
		СтрокаОпераций.УчетнаяЦена	= СтрокаТЧ.УчетнаяЦена;
		СтрокаОпераций.Стоимость	= СтрокаТЧ.Стоимость;
		СтрокаОпераций.Узел			= Ложь;
...
//	ТаблицаОпераций - ТаблицаЗначений операций.
//  
Процедура Разузлование(СтруктураСостава, ТаблицаСостава, ТаблицаОпераций) Экспорт
	...
	ВыполнитьРазузлование(СтруктураСостава, ТаблицаСостава, ТаблицаОпераций);
		...
		Пока ВыборкаНоменклатуры.Следующий() Цикл
			...
			Если СтрокиРекурсивногоВхождения.Количество() <> 0 Тогда
				...
				Для каждого СтрокаРекВх Из СтрокиРекурсивногоВхождения Цикл
					...
					СообщитьОбОшибке(СтруктураСостава.Объект, ТекстСообщения);
			...
			НоваяСтрокаВход.ТипСтрокиСостава	= ВыборкаНоменклатуры.ТипСтрокиСостава;
			НоваяСтрокаВход.Номенклатура		= ВыборкаНоменклатуры.Номенклатура;
			НоваяСтрокаВход.Характеристика		= ВыборкаНоменклатуры.Характеристика;
			НоваяСтрокаВход.ЕдиницаИзмерения	= ВыборкаНоменклатуры.ЕдиницаИзмерения;
			...
			НоваяСтрокаВход.КоличествоПродукции = ВыборкаНоменклатуры.КоличествоПродукции;
			НоваяСтрокаВход.Спецификация		= ВыборкаНоменклатуры.Спецификация;
...
Процедура ИзменитьПодчиненнуюСчетФактуру(ДокументОснование, Полученный = Ложь) Экспорт
	...
	СчетФактура = УправлениеНебольшойФирмойСервер.ПолучитьПодчиненныйСчетФактуру(ДокументОснование, Полученный);
...
Процедура ЗаполнитьПараметрыОтправки(ПараметрыОтправки, МассивОбъектов, КоллекцияПечатныхФорм) Экспорт
	...
	ПараметрыОтправки.Получатель = УправлениеНебольшойФирмойКонтактнаяИнформацияСервер.ПодготовитьЭлектронныеАдресаПолучателей(Получатели, Истина);
...
//
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПоставщиками(МенеджерВременныхТаблиц, СЗачетомАванса, НомерЗапроса) Экспорт
...
Процедура ЗаполнитьРасшифровкуПлатежаРасход(ТекущийОбъект, Компания = Неопределено, СтавкаНДСПоУмолчанию = Неопределено, Курс = Неопределено, Кратность = Неопределено, Договор = Неопределено) Экспорт
	...
	Если СтавкаНДСПоУмолчанию = Неопределено Тогда
		Если ТекущийОбъект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДС Тогда
			СтавкаНДСПоУмолчанию = ТекущийОбъект.Организация.СтавкаНДСПоУмолчанию;
...
Функция ОтказСнятьФункциональнаяОпцияУчетЦенКонтрагентов() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстОшибки = НСтр("ru = 'В базе содержатся договоры контрагентов с установлинными видами входящих цен! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьСкидкиНаценки() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьДисконтныеКарты") Тогда
		...
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", Символы.ПС) + 
			НСтр("ru = 'Включена опция ""Использовать дисконтные карты""! Снятие флага запрещено!'");
...
Функция ОтказСнятьФункциональнаяОпцияИспользоватьАвтоматическиеСкидкиНаценки() Экспорт
	...
	Пока ВыборкаАвтоматическиеСкидки.Следующий() Цикл
		...
		Если ТаблицаСсылок.Количество() > 0 Тогда
			...
			ТекстОшибки = НСтр("ru = 'В базе используются виды автоматических скидок, наценок! Снятие опции запрещено!'");
			Прервать;
...
Функция ОтказСнятьИспользоватьНесколькоОрганизаций() Экспорт
	...
	Запрос.Текст = 
		...
		|	Справочник.Организации КАК Организации
		|ГДЕ
		|	Организации.Ссылка <> ЗНАЧЕНИЕ(Справочник.Организации.ОсновнаяОрганизация)";
...
Функция ОтказСнятьУчетПоКомпании() Экспорт
	...
	Счетчик = 0;
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ПроверяемаяОрганизация", Константы.Компания.Получить());
...
Функция ДанныеЕГАИСДостаточны(ТаблицаТоваров, ДокументОбъект) Экспорт

	ДанныеЕГАИСДостаточны = Истина;
	
	ПроверитьДанныеЕГАИС(ТаблицаТоваров, ДанныеЕГАИСДостаточны, ДокументОбъект);
	
	Возврат ДанныеЕГАИСДостаточны;
...
Процедура ПроверитьДанныеЕГАИС(ТаблицаТоваров, ДанныеЕГАИСДостаточны, ДокументОбъект) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ТекстТекущейОшибки,
			ДокументОбъект,
			"Объект.Товары[" + (Выборка.НомерСтроки - 1) + "]." + ОткрываемоеПоле);
...
Функция ПодготовкаДанныхДляПробитияЧекаККМ(ДокументОбъект, ДанныеЕГАИСДостаточны, ЕстьАлкогольнаяПродукцияЕГАИС) Экспорт
	...
	ЕстьАлкогольнаяПродукцияЕГАИС = ЕстьАлкогольнаяПродукцияЕГАИС(ДокументОбъект.Запасы);
	
	Если ЕстьАлкогольнаяПродукцияЕГАИС Тогда
		ПродажаСПроверкойЕГАИС = ИспользуетсяРегистрацияРозничныхПродажВЕГАИС(ДокументОбъект.Дата);
	Иначе
...
Функция ПодготовкаДанныхДляОтправкиВЕГАИС(ДокументОбъект, ДанныеЕГАИСДостаточны) Экспорт
