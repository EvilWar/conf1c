	...
	Если СтруктураФорма4ФСС.ДокументОтчетности <> Неопределено Тогда
		СтруктураФорма4ФСС.ПараметрыФормыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			...
			Если (ПериодРасчета >= '20130630' И СтруктураФорма4ФСС.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2012Кв1")
				...
				ИЛИ (ПериодРасчета >= '20170101' И СтруктураФорма4ФСС.ПараметрыФормыДокумента.ВыбраннаяФорма = "ФормаОтчета2016Кв3")Тогда
				// Форма документа отчетности не соответствует периоду формирования
...
Процедура ВыполнитьФормированиеПоРасходамНаТовары(Организация, ПериодФормирования, Результат, МенеджерВТ)
	...
	|	ПартииТоваровОбороты.Регистратор КАК ПервичныйДокумент,
	|	ПартииТоваровОбороты.Регистратор.Дата КАК ДатаПервичногоДокумента,
	|	ПартииТоваровОбороты.Регистратор.Номер КАК НомерПервичногоДокумента,
...
Процедура ВыполнитьФормированиеПоРасходамНаУслуги(Организация, ПериодФормирования, Результат, МенеджерВТ)
	...
	|	ВТРасходСоСчета.РасходБаза * ЕСТЬNULL(КурсыВалютПоДням.Коэффициент, 1),
	|	ВТРасходСоСчета.РасходВсего * ЕСТЬNULL(КурсыВалютПоДням.Коэффициент, 1),
	|	ВТРасходСоСчета.СуммаНДС * ЕСТЬNULL(КурсыВалютПоДням.Коэффициент, 1),
	...
	|	ВТПоступлениеНаСчет.РасходБаза * ЕСТЬNULL(КурсыВалютПоДням.Коэффициент, 1),
	|	ВТПоступлениеНаСчет.РасходВсего * ЕСТЬNULL(КурсыВалютПоДням.Коэффициент, 1),
	|	ВТПоступлениеНаСчет.СуммаНДС * ЕСТЬNULL(КурсыВалютПоДням.Коэффициент, 1),
	...
	|	ВТПоступлениеВКассу.РасходБаза * ЕСТЬNULL(КурсыВалютПоДням.Коэффициент, 1),
	|	ВТПоступлениеВКассу.РасходВсего * ЕСТЬNULL(КурсыВалютПоДням.Коэффициент, 1),
	|	ВТПоступлениеВКассу.СуммаНДС * ЕСТЬNULL(КурсыВалютПоДням.Коэффициент, 1),
	...
	|	ВТАвансовыйОтчет.РасходБаза * ЕСТЬNULL(КурсыВалютПоДням.Коэффициент, 1),
	|	ВТАвансовыйОтчет.РасходВсего * ЕСТЬNULL(КурсыВалютПоДням.Коэффициент, 1),
	|	ВТАвансовыйОтчет.СуммаНДС * ЕСТЬNULL(КурсыВалютПоДням.Коэффициент, 1),
...
Процедура ВыполнитьФормированияДвиженийПартионногоУчета(ГраницаВыполнения) Экспорт
	...
	Пока ВыборкаОрганизаций.Следующий() Цикл
		...
		ИсходнаяТаблицаДвижений = ПолучитьИсходнуюТаблицуДляФормированияДвиженийПоПартиям(ВыборкаОрганизаций.Ссылка, ГраницаНачала, ГраницаВыполнения);
		
		// формирование дерева результат и таблицы партионного учета
		ОбработатьТаблицуДвижений(ИсходнаяТаблицаДвижений,ДвиженияДокументовПартий,ТаблицаПартионногоУчета);
...
Функция ПолучитьИсходнуюТаблицуДляФормированияДвиженийПоПартиям(Организация, ГраницаНачала, ГраницаВыполнения)
	...
	|	ИнтересующиеДокументыПоступлений.ДатаДокумента КАК Дата,
	|	ИнтересующиеДокументыПоступлений.ДатаСортировки,
	|	ИнтересующиеДокументыПоступлений.ТоварныйДокумент КАК Регистратор,
	|	ИнтересующиеДокументыПоступлений.ТоварныйДокумент КАК ДокументПартии,
	|	ИнтересующиеДокументыПоступлений.Организация КАК Организация,
	|	ИнтересующиеДокументыПоступлений.Номенклатура КАК Номенклатура,
	|	СУММА(ИнтересующиеДокументыПоступлений.Количество) КАК Количество,
	|	СУММА(ИнтересующиеДокументыПоступлений.Сумма) КАК Сумма,
	|	СУММА(ИнтересующиеДокументыПоступлений.СуммаНДС) КАК СуммаНДС
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		Закупки.ДатаДокумента КАК ДатаДокумента,
	...
	|				И Закупки.УчитыватьВНУ) КАК ИнтересующиеДокументыПоступлений
	|
	|СГРУППИРОВАТЬ ПО
	|	ИнтересующиеДокументыПоступлений.Организация,
	|	ИнтересующиеДокументыПоступлений.ДатаДокумента,
	|	ИнтересующиеДокументыПоступлений.ТоварныйДокумент,
	|	ИнтересующиеДокументыПоступлений.ДатаСортировки,
	|	ИнтересующиеДокументыПоступлений.Номенклатура,
	|	ИнтересующиеДокументыПоступлений.ТоварныйДокумент
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	""ВозвратПоставщику"",
	|	ИнтересующиеДокументы.ДатаДокумента,
	|	ИнтересующиеДокументы.ДатаСортировки,
	|	ИнтересующиеДокументы.ТоварныйДокумент,
	|	NULL,
	|	ИнтересующиеДокументы.Организация,
	|	ИнтересующиеДокументы.Номенклатура,
	|	-ИнтересующиеДокументы.Количество,
	|	-ИнтересующиеДокументы.Сумма,
	|	-ИнтересующиеДокументы.СуммаНДС
	|ИЗ
	|	(ВЫБРАТЬ
	|		Закупки.ТоварныйДокумент КАК ТоварныйДокумент,
	|		Закупки.Организация КАК Организация,
	...
	|			ПО Закупки.ТоварныйДокумент = РасходнаяНакладная.Ссылка
	|				И (Закупки.Период МЕЖДУ &ГраницаНачала И &ГраницаВыполнения)
	|				И (Закупки.Организация = &Организация)
	|				И (Закупки.Сумма < 0)
	|				И (Закупки.ЭтоТоварыКРеализации)) КАК ИнтересующиеДокументы
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		Продажи.Регистратор КАК ТоварныйДокумент,
	|		&Организация КАК Организация,
	|		Продажи.Номенклатура КАК Номенклатура,
	|		Продажи.Количество КАК Количество,
	|		Продажи.Регистратор.Дата КАК ДатаДокумента,
	|		Продажи.Регистратор.Дата КАК ДатаСортировки
	|	ИЗ
	...
	|		И Продажи.Количество <> 0) КАК ИнтересующиеДокументы
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаСортировки");
	...
	ТаблицаДвижений.Индексы.Добавить("Номенклатура,ДокументПартии");
	ТаблицаДвижений.Индексы.Добавить("Номенклатура");
...
Процедура ОбработатьТаблицуДвижений(ИсходнаяТаблицаДвижений,ДвиженияДокументовПартий,ТаблицаПартионногоУчета)
	...
	Для Каждого СтрокаДвижения Из ИсходнаяТаблицаДвижений Цикл
		...
		Если ПредыдущееПоступление <> СтрокаДвижения.Регистратор Тогда // перешли к обработке следующего регистратора
			...
			ДвиженияДокументовПартий[ИндексПартии][0] = СтрокаДвижения.Регистратор;
		...
		ИначеЕсли СтрокаДвижения.ВидЗаписи = "Продажа" Тогда
			...
			РаспределитьСтрокуДвиженияПоПартиям(ТаблицаПартионногоУчета,ДвиженияДокументовПартий[ИндексПартии][1],СтрокаДвижения);
			
		ИначеЕсли СтрокаДвижения.ВидЗаписи = "ВозвратПокупателя" Тогда
			
			ОбработатьСтрокуДвиженияВозвратаОтПокупателя(ТаблицаПартионногоУчета,ДвиженияДокументовПартий, ДвиженияДокументовПартий[ИндексПартии][1], СтрокаДвижения);
...
//
Процедура СторнироватьТаблицуПартийПоДвижению(ТаблицаПартионногоУчета, СтрокаДвижения)
	...
	Если НайденныеСтроки.Количество() > 0 Тогда
		НайденныеСтроки[0].Количество =  НайденныеСтроки[0].Количество - СтрокаДвижения.Количество;
		НайденныеСтроки[0].Сумма =  НайденныеСтроки[0].Сумма - СтрокаДвижения.Сумма;
		НайденныеСтроки[0].СуммаНДС =  НайденныеСтроки[0].СуммаНДС - СтрокаДвижения.СуммаНДС;
	КонецЕсли;
...
//		СтрокаДвижений - СтрокаТаблицыЗначений - строка,содержащая информация по списываемому товару по партии
//
Процедура ОбработатьСтрокуДвиженияВозвратаОтПокупателя(ТаблицаПартионногоУчета,ОбщийМассивДвижений,ТаблицаДвижений, СтрокаДвижения)
	
	ДокументРеализации = СтрокаДвижения.ДокументПартии;
	...
	Для кк = -(КоличествоСтрок-1) По 0 Цикл
		...
		СторнироватьТаблицуПартийПоДвижению(ТаблицаПартионногоУчета, СтрокаРезультата);
