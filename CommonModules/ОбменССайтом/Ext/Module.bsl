...
Процедура ЗаданиеВыполнитьОбмен(КодУзлаОбмена) Экспорт
	...
	Если УзелОбмена.ВыгружатьНаСайт Тогда
		...
		СтруктураПодключения.Вставить("АдресСайта",УзелОбмена.АдресСайта);
		СтруктураПодключения.Вставить("ИмяПользователя",УзелОбмена.ИмяПользователя);
...
Функция ТекстФайлаОбмена(УзелОбмена, ВыгружатьТолькоИзменения = Истина) Экспорт
	...
	Если ОсновныеПараметры.ОбменТоварами Тогда 
		...
		Для Каждого СтрокаТаблицыКаталогов Из ТаблицаКаталогов Цикл
			...
			URIПространстваИмен = "urn:1C.ru:commerceml_208";
...
Процедура ПодготовитьДанныеДляВыгрузки(Параметры)
	...
	Параметры.Вставить("ВыборкаСвойствХарактеристикДляКлассификатора", 
		МассивРезультатовЗапроса[32].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам));
	...
	Если Параметры.ВыгружатьКартинки Тогда
		...
		РезультатЗапросаКартинкиХарактеристик = МассивРезультатовЗапроса[34];
	...
	ТекстЗапроса = ТекстЗапроса + Символы.ПС + ";" + Символы.ПС
	  + "ВЫБРАТЬ
		...
		Если Параметры.ВыгружатьКартинки Тогда
			...
			|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПрисоединенныеФайлы КАК ПрисоединенныеФайлы
			|			ПО ХарактеристикиНоменклатурыПрисоединенныеФайлы.Ссылка = ПрисоединенныеФайлы.ПрисоединенныйФайл
...
Процедура ДобавитьНоменклатуруXDTO(ТоварыXDTO, ПакетCML, ДополнительныеДанные, Параметры)
	...
	Если НЕ (Параметры.Свойство("ОбменЧерезВебСервис") И Параметры.ОбменЧерезВебСервис=Истина) Тогда
		...
		Если ЗначениеЗаполнено(Параметры.ВыборкаНоменклатуры.КатегорияНоменклатуры) Тогда
			ЗаполнитьИДСайтаДляОбъектаXDTO(ТоварXDTO, Параметры.ВыборкаНоменклатуры.КатегорияНоменклатуры,, Параметры, "Категория")
		КонецЕсли;
	КонецЕсли;
...
Функция ПодготовитьДанныеXDTOПакетПредложений(Параметры, ПакетCML)
	...
	Если Параметры.УзелОбмена.РежимВыгрузкиЦен = 1 Тогда //Обновление цен, без товаров
	...
	Иначе	
		...
		Если Параметры.УзелОбмена.ПротоколОбменаCMS = Перечисления.ПротоколыОбменаCMS.Битрикс Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			Если Не Параметры.ОбменЧерезВебСервис Тогда
				...
				Для Каждого СтрокаСвойства Из НайденнаяСтрока.Строки Цикл
					...
					//ЗначСвойстваXDTO.Наименование = ФорматНаименованияДляXDTO(СтрокаСвойства.Наименование);
					
					ТипЗначенияСвойства = СтрокаСвойства.Свойство.ТипЗначения;//ТипЗнч(СтрокаСвойства.Значение);
...
Функция ЗаполнитьЗаказДаннымиДокументаXDTO(ДокументОбъект, ДокументXDTO, СвойстваЗаказа, Параметры, ОписаниеОшибки)
	...
	Если ТипЗнч(СвойстваЗаказа) = Тип("Соответствие") Тогда
		...
		ДобавитьСтрокуИнформацииПоКонтрагенту("Комментарии покупателя", КомментарийПокупателя, ДокументОбъект.ИнформацияПоКонтрагенту);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеКонтрагентаЗаказа(КонтрагентXDTO, ИнформацияПоКонтрагенту)
	...
	ИначеЕсли НЕ ЭтоЮрЛицо И ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "АдресРегистрации")
		...
		ДобавитьСтрокуИнформацииПоКонтрагенту("Адрес регистрации", КонтрагентXDTO.АдресРегистрации.Представление, ИнформацияПоКонтрагенту);
...
Процедура ДобавитьЗначенияРеквизитовЗаказаXDTO(ДокументXDTO, ПакетCML, ДанныеДокумента, Параметры)
	...
	Если Параметры.Свойство("ТаблицаСоответствияСтатусовЗаказов")
		...
		НайденноеСоответствие = Параметры.ТаблицаСоответствияСтатусовЗаказов.Найти(ДанныеДокумента.СостояниеЗаказа, "СостояниеЗаказаПокупателя");
		Если НайденноеСоответствие <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(НайденноеСоответствие.СтатусЗаказаНаСайте) Тогда
				...
				Если Параметры.УзелОбмена.ПротоколОбменаCMS = Перечисления.ПротоколыОбменаCMS.Битрикс 
					И ЗначениеЗаполнено(НайденноеСоответствие.ИДСтатусаЗаказаНаСайте) Тогда
					
					СтатусНаСайте = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						"[%1] %2",
						НайденноеСоответствие.ИДСтатусаЗаказаНаСайте,
						НайденноеСоответствие.СтатусЗаказаНаСайте);
				КонецЕсли;
...
Функция ОтправитьФайлНаСервер(ПолноеИмяФайла, Соединение, ПараметрыЗапроса="", Заголовки="", ОписаниеОшибки)
	...
	Попытка
		HTTPОтветСервера = Соединение.ОтправитьДляОбработки(ПолноеИмяФайла, СокрЛП(ПараметрыЗапроса), ИмяФайлаОтвета, СокрЛП(Заголовки));
	Исключение
		ДобавитьОписаниеОшибки(ОписаниеОшибки, ОписаниеИсключительнойОшибки());
	КонецПопытки;
	...
	Если ФайлОтвета.Существует() Тогда
		...
		КодировкаXML = КодировкаФайла(ИмяФайлаОтвета);
		...
		Если ТекстОтвета.КоличествоСтрок()>0 Тогда
			...
			Если СтрНайти(ОтветСервера,"�")>0 Тогда
				...
				Если HTTPОтветСервера<>Неопределено И HTTPОтветСервера.Заголовки.Получить("Content-Type")<>Неопределено Тогда
...
Функция ИдентификаторОбъектаДляВыгрузки(Ссылка, Характеристика, Параметры)
	...
	ИначеЕсли Параметры.УзелОбмена.ПротоколОбменаCMS = Перечисления.ПротоколыОбменаCMS.UMI Тогда
...
Функция ЗаполнитьИДСайтаДляОбъектаXDTO(ОбъектXDTO, ВыгружаемаяСсылка, Характеристика=Неопределено, Параметры, ИмяПоляИд="Ид")
	...
	ИначеЕсли Параметры.УзелОбмена.ПротоколОбменаCMS = Перечисления.ПротоколыОбменаCMS.UMI Тогда
		...
		ИдентификаторЭлемента = СформироватьИдентификаторСайтаОбъекта(ВыгружаемаяСсылка, Характеристика, Параметры.УзелОбмена, Ложь).ИдСайта;
		Если ЗначениеЗаполнено(ИдентификаторЭлемента) Тогда
			//ИдентификаторЭлемента = ОбменССайтомПовтИсп.СформироватьУникальныйИдентификаторОбъекта(ВыгружаемаяСсылка, Характеристика);
			ОбъектXDTO.ИдСайта = ИдентификаторЭлемента;
...
Процедура ВыполнитьОбменИнтерактивно(ПараметрыОбмена, АдресРезультата = Неопределено) Экспорт
	...
	ВыполнитьОбмен(УзелОбмена, РежимЗапускаОбмена, ВыгружатьТолькоИзменения);
