...
Процедура ВыполнитьРегистрациюНаСервере(РеквизитыКассы, Режим, ЕстьОшибки, СообщениеОбОшибке, ВестиЛог = Ложь, ОбластьДанных = 0) Экспорт
	...
	ВыполнитьПодключениеКассовыхАппратов(КассыДляРегистрации, Режим,
...
Процедура ВыполнитьДействияПриУстановкеПараметровСеанса(ИменаПараметровСеанса) Экспорт
	...
	ОбменСКассовымСерверомШтрихМПереопределяемый.ВыполнитьДействияПриУстановкеПараметровСеанса(ИменаПараметровСеанса);
КонецПроцедуры
...
Процедура УстановитьИспользованиеРегламентногоЗадания() Экспорт
	...
	Если НЕ ИспользоватьОбменССерверомШтрихМ Тогда
		Возврат;
	КонецЕсли;
	
	УзелОбмена = ОбменСКассовымСерверомШтрихМПовтИсп.УзелОбменаШтрих();
	
	ВыполнятьОбменССерверомШтрихМ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелОбмена, "ВыполнятьОбменССерверомШтрихМ");
...
Процедура ЗарегистрироватьТовары(Соединение, Настройки, ТоварыДляОбмена, Отказ, СообщениеОбОшибке)
	...
	Для каждого СтрокаТоваров Из Товары Цикл
		...
		РеквизитыТоваров.Вставить("alcohol_excisable", СтрокаТоваров.АлкогольнаяПродукция);
