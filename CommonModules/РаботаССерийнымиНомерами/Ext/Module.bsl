...
Функция СтрокаСерийныеНомераИзВыборки(ВыборкаСтрокСерийныеНомера, КлючСвязи) Экспорт
	...
	Если ТипЗнч(ВыборкаСтрокСерийныеНомера) = Тип("ТаблицаЗначений") Тогда
		...
		ПечатьДокументовУНФ.ПредставлениеСерийногоНомера(ВыборкаСтрокСерийныеНомера, КлючСвязи);
...
Процедура ПроверкаЗаполненияСерийныхНомеров(Отказ, Знач Запасы, Знач СерийныеНомера, СтруктурнаяЕдиница, Знач вхФорма, ИмяПоляКлючСвязи="КлючСвязи", ИмяПоляНоменклатура = "Номенклатура") Экспорт
	...
	Если ИспользованиеСерийныхНомеров = Истина Тогда
		...
		Для каждого СтрокаЗапасы Из Запасы Цикл
			...
			Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктурнаяЕдиница,"ОрдерныйСклад") И СтрокаЗапасы[ИмяПоляНоменклатура].ВидМаркировки = Перечисления.ВидыМаркировки.МаркируемаяПродукция Тогда
				Продолжить;
			КонецЕсли;
			
			Если СтрокаЗапасы[ИмяПоляНоменклатура].ИспользоватьСерийныеНомера Тогда
