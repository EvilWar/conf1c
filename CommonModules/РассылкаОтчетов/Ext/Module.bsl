...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено)
	...
	Если ПараметрыОтчета.СКД Тогда
		ПользовательскиеНастройкиКД = ПараметрыОтчета.КомпоновщикНастроекКД.ПользовательскиеНастройки;
		ПараметрыОтчета.КомпоновщикНастроекКД.ЗагрузитьПользовательскиеНастройки(ПользовательскиеНастройкиКД);
		ПараметрыОтчета.КомпоновщикНастроекКД.ЗагрузитьНастройки(ПараметрыОтчета.КомпоновщикНастроекКД.ПолучитьНастройки());
		ПараметрыФормирования.Вставить("НастройкиКД", ПараметрыОтчета.КомпоновщикНастроекКД.Настройки);
	КонецЕсли; 
	// Конец УНФ
	...
	Если Получатель = Неопределено Тогда
		...
		Если ПараметрыДоставки.ЗаполнитьПолучателяВШаблонеТемы Или ПараметрыДоставки.ЗаполнитьПолучателяВШаблонеСообщения Тогда
			...
			Для Каждого КлючИЗначение Из ПараметрыДоставки.Получатели Цикл
				...
				// Отправляем письмо
				ЭлектроннаяПочтаУНФ.ОтправитьПочтовоеСообщение(ПараметрыПисьма, ПараметрыДоставки.УчетнаяЗапись);
			КонецЦикла;
		Иначе
