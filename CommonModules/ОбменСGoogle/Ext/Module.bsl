...
Функция ЗагрузитьЗаголовкиСообщенийGmail(Знач УчетнаяЗапись, Знач СеансовыеДанные, Знач РежимЗагрузки, Знач Пользователь, Знач ОтборПоКонтакту = Неопределено) Экспорт
	...
	МетаданныеСообщений = МетаданныеСообщений(ИдентификаторыСообщений, СеансовыеДанные);
	
	АдресаУчастников = АдресаУчастников(МетаданныеСообщений);
	...
	Для Каждого ТекМетаданные Из МетаданныеСообщений Цикл
		
		ДлительныеОперации.СообщитьПрогресс(Окр(РезультатЗагрузкиЗаголовков.Загружено / МетаданныеСообщений.Количество() * 100),
		СтрШаблон(НСтр("ru = '%1: загружено %2 из %3'"),
		ЗаголовокПрогресса,
		РезультатЗагрузкиЗаголовков.Загружено,
		МетаданныеСообщений.Количество()));
		
		Если СобытияПоИдентификаторам[ТекМетаданные.Идентификатор] <> Неопределено Тогда
			РегистрыСведений.ИдентификаторыСобытий.ЗаписатьИдентификатор(УчетнаяЗапись, СобытияПоИдентификаторам[ТекМетаданные.Идентификатор], ТекМетаданные.Идентификатор);
			Продолжить;
		КонецЕсли;
		
		Если ЭтоСпам(ТекМетаданные.Метки) Тогда
			Продолжить;
		КонецЕсли;
		
		СсылкаНаСобытие = ЗаписатьСобытиеПоМетаданнымGMail(ТекМетаданные, УчетнаяЗапись, СопоставленныеКонтактыПоАдресамЭП, Пользователь);
		
		ЭлектроннаяПочтаУНФ.ОбработатьРезультатЗагрузкиЗаголовков(РезультатЗагрузкиЗаголовков, СсылкаНаСобытие, ТекМетаданные.Дата, РежимЗагрузки);
		
		Если РежимЗагрузки = ЭлектроннаяПочтаУНФКлиентСервер.РежимЗагрузкиНовыеСообщения() Тогда
			ИдентификаторИсторииДляЗаписи = НаиболееПозднийИдентификаторИстории(ТекМетаданные.ИдентификаторИстории, ИдентификаторИсторииДляЗаписи);
		КонецЕсли;
...
Процедура ЗагрузитьСодержаниеСообщенияGmail(Знач Идентификатор, Знач СеансовыеДанные, АдресХранилища) Экспорт
	
	СодержаниеСообщения = СодержаниеСообщения(Идентификатор, СеансовыеДанные);
	
	СодержаниеСообщенияДвоичныеДанные = Base64URLЗначение(СодержаниеСообщения["raw"]);
	
	ТекИнтернетПочтовоеСообщение = Новый ИнтернетПочтовоеСообщение;
	ТекИнтернетПочтовоеСообщение.УстановитьИсходныеДанные(СодержаниеСообщенияДвоичныеДанные);
...
Функция СоздатьУчетнуюЗаписьGoogle(СеансовыеДанные) Экспорт
	...
	ИмяОтправителя = ДанныеАвторизованногоПользователя(СеансовыеДанные)["displayName"];
...
Функция ЗаполнитьОписаниеПолейДляАвтоПодбораРекурсивно(ОписаниеПолей, Знач ПодчиненныеЭлементы, Знач Форма)
...
Функция МетаданныеСообщений(Идентификаторы, Знач СеансовыеДанные)
	...
	ПакетныйЗапросHTTP = Новый HTTPЗапрос;
	ПакетныйЗапросHTTP.АдресРесурса = "/batch";
	ПакетныйЗапросHTTP.Заголовки["Authorization"] = СтрШаблон("%1 %2", СеансовыеДанные["token_type"], СеансовыеДанные["access_token"]);
	ПакетныйЗапросHTTP.Заголовки["Content-Type"] = СтрШаблон("multipart/mixed; boundary=%1", ГраницаСекций);
	ПакетныйЗапросHTTP.УстановитьТелоИзСтроки(ТелоПакетногоЗапросаМетаданныеСообщений(Идентификаторы, ГраницаСекций));
	
	ПакетныйОтветHTTP = HTTPСоединение.ОтправитьДляОбработки(ПакетныйЗапросHTTP);
	
	КомпонентыПакетногоОтвета = КомпонентыПакетногоОтвета(ПакетныйОтветHTTP, Неопределено);
	
	Результат = МетаданныеСообщенийИзПакетногоОтвета(КомпонентыПакетногоОтвета);
	
	Возврат Результат;
	
КонецФункции

Функция ТелоПакетногоЗапросаМетаданныеСообщений(Идентификаторы, ГраницаСекций)
	...
	Для Каждого ТекИдентификатор Из Идентификаторы["messages"] Цикл
		...
		|Content-Type: application/http
		|
		|GET /gmail/v1/users/me/messages/%3?format=metadata",
		?(ЗначениеЗаполнено(Результат), Результат + Символы.ПС + Символы.ПС, ""),
		ГраницаСекций,
		ТекИдентификатор["id"]);
...
Функция КомпонентыПакетногоОтвета(ПакетныйОтветHTTP, Менеджер)
	...
	ContentType = СтроковыеФункцииКлиентСервер.ПолучитьПараметрыИзСтроки(ПакетныйОтветHTTP.Заголовки["Content-Type"]);
...
Функция СтруктураКомпонентаПакета(Компонент, Менеджер)
	...
	Для ИндексСтрок = ИндексСтрок По СтрЧислоСтрок(Компонент) Цикл
	...
	Результат.Вставить("Содержимое", ПрочитатьJSON(ЧтениеJSON, Истина));
	
	Возврат Результат;
	
КонецФункции

Функция МетаданныеСообщенийИзПакетногоОтвета(КомпонентыПакетногоОтвета)
	...
	Для Каждого КомпонентПакетногоОтвета Из КомпонентыПакетногоОтвета Цикл
		...
		МетаданныеСообщения = НовыеМетаданныеСообщения();
		МетаданныеСообщения.Идентификатор = КомпонентПакетногоОтвета.Значение.Содержимое["id"];
		МетаданныеСообщения.ИдентификаторИстории = КомпонентПакетногоОтвета.Значение.Содержимое["historyId"];
		МетаданныеСообщения.Сниппет = КомпонентПакетногоОтвета.Значение.Содержимое["snippet"];
		МетаданныеСообщения.Дата = ДатаИзВремениUnix(КомпонентПакетногоОтвета.Значение.Содержимое["internalDate"]);
		МетаданныеСообщения.ВходящееИсходящееСобытие = ВходящееИсходящееСобытие(КомпонентПакетногоОтвета.Значение.Содержимое["labelIds"]);
		МетаданныеСообщения.Метки = КомпонентПакетногоОтвета.Значение.Содержимое["labelIds"];
		...
		Для Каждого ТекЗаголовок Из Заголовки Цикл
			...
			Если НРег(ТекЗаголовок["name"]) = "subject" Тогда
				МетаданныеСообщения.Тема = ТекЗаголовок["value"];
			КонецЕсли;
			
			Если НРег(ТекЗаголовок["name"]) = "message-id" Тогда
				МетаданныеСообщения.ИдентификаторСообщения = ТекЗаголовок["value"];
			КонецЕсли;
			
			Если НРег(ТекЗаголовок["name"]) = "in-reply-to" Тогда
				МетаданныеСообщения.ИдентификаторОснования = ТекЗаголовок["value"];
			КонецЕсли;
			
			Если НРег(ТекЗаголовок["name"]) = "references" Тогда
				МетаданныеСообщения.ИдентификаторыОснований = ТекЗаголовок["value"];
			КонецЕсли;
			
			Если НРег(ТекЗаголовок["name"]) = "from" Тогда
				ЗаполнитьЗначенияСвойств(
				МетаданныеСообщения.Отправитель,
				ЭлектроннаяПочтаУНФ.АдресЭлектроннойПочтыИзСтроки(ТекЗаголовок["value"]));
			КонецЕсли;
			
			Если НРег(ТекЗаголовок["name"]) = "to" Тогда
				МетаданныеСообщения.Получатели = ЭлектроннаяПочтаУНФ.АдресаЭлектроннойПочтыИзСтроки(ТекЗаголовок["value"]);
			КонецЕсли;
			
		КонецЦикла;
		
		Результат.Добавить(МетаданныеСообщения);
...
Функция АдресаУчастников(Знач МетаданныеСообщений)
...
Функция НовыеМетаданныеСообщения()
...
Функция ЗаписатьСобытиеПоМетаданнымGMail(МетаданныеСообщения, УчетнаяЗапись, СопоставленныеКонтакты, Пользователь)
	
	Результат = Документы.Событие.ПустаяСсылка();
	
	ИдентификаторыСвязанныхСообщений = ЭлектроннаяПочтаУНФ.ИдентификаторыСвязанныхСообщений(МетаданныеСообщения.ИдентификаторыОснований);
	...
	Попытка
		...
		СобытияПоИдентификатору = РегистрыСведений.ИдентификаторыСобытий.СобытияПоИдентификаторам(УчетнаяЗапись, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(МетаданныеСообщения.Идентификатор));
		Если ЗначениеЗаполнено(СобытияПоИдентификатору) Тогда
			...
			МетаданныеСообщения.Идентификатор);
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
		
		НовоеСобытие = Документы.Событие.СоздатьДокумент();
		НовоеСобытие.ДополнительныеСвойства.Вставить("Идентификатор", МетаданныеСообщения.Идентификатор);
		НовоеСобытие.ДополнительныеСвойства.Вставить("ИдентификаторыСообщений",
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(МетаданныеСообщения.ИдентификаторСообщения));
		
		ЗначенияЗаполнения = Новый Структура;
		ЗначенияЗаполнения.Вставить("Дата", МетаданныеСообщения.Дата);
		ЗначенияЗаполнения.Вставить("НачалоСобытия", МетаданныеСообщения.Дата);
		...
		ЗначенияЗаполнения.Вставить("Тема", МетаданныеСообщения.Тема);
		ЗначенияЗаполнения.Вставить("ВходящееИсходящееСобытие", МетаданныеСообщения.ВходящееИсходящееСобытие);
		...
		ЭлектроннаяПочтаУНФ.ЗаполнитьКонтакты(НовоеСобытие, МетаданныеСообщения, СопоставленныеКонтакты);
		
		ТемаИСодержание = Документы.Событие.ТемаИСодержание(
		МетаданныеСообщения.Тема,
		МетаданныеСообщения.Сниппет,
		"");
		
		НовоеСобытие.ДополнительныеСвойства.Вставить("ТемаИСодержание", ТемаИСодержание);
		
		НовоеСобытие.ЗаполнитьДокументыОснования(МетаданныеСообщения.ИдентификаторОснования, ИдентификаторыСвязанныхСообщений);
